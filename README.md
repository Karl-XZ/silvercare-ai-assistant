...
- **Android 应用与端侧 AI 链路**：语音优先界面、居家巡路、找物、精确引导、跌倒风险确认、照护记录、管理端复核，以及本地/云端可切换的运行方式。
- **端侧推理与性能资料**：MNN Runtime、DAMO-YOLO、Qwen 文本模型、本地语音识别、Arm SME2 自动检测与调优报告。
- **可穿戴硬件 V1 资料**：Plan A（ESP32-S3）与 Plan B（BK7258）两套眼镜端方案，包含冻结的业务 GPIO / Pin Matrix、信号网、供电树、BOM、设计约束与问题台账。
- **可复现实验资料**：脱敏真实居家场景 benchmark、trace 协议、评分规则、baseline 与自动化测试脚本。

### 2026-08-04 Android 调试安装包

可直接下载 [`releases/银龄智护-20260804.apk`](releases/%E9%93%B6%E9%BE%84%E6%99%BA%E6%8A%A4-20260804.apk)。该文件通过 Git LFS 保存；在 GitHub 网页中打开后点击 **Download raw file** 即可下载。

- 应用包名：`com.medicalinsurance.longtermcare`
- 应用版本：`1.0.0`
- 最低系统：Android 6.0（API 23）
- 支持架构：`arm64-v8a`、`x86_64`
- 文件大小：约 71 MB
- SHA-256：`3E98461062F0BB0CB33E6E77CF2D07353050D5985016072D22201B5822678454`

安装方式：下载完成后在 Android 文件管理器中打开 APK，按系统提示允许安装即可。首次使用需要授予相机和麦克风权限；端侧大模型、语音模型和其他可选资源由应用内下载器按需下载，安装 APK 本身不包含这些大模型权重。

> 说明：该 APK 是 2026-08-04 生成的 Android 调试包。仓库主线持续维护应用源码、端侧模型集成、公开 benchmark 与可穿戴硬件资料；如需使用后续源码构建新的安装包，请按下文的构建步骤执行。
