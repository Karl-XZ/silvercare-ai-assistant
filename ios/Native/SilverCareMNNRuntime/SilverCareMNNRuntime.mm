#include "../SilverCareMNNRuntimeABI.h"
...
void applyLlmTuningConfig(MNN::Transformer::Llm* llm, const std::string& tuningConfigJson) {
    ...
    if (!llm->set_config(normalized)) {
        throw std::runtime_error("Failed to apply MNN LLM tuning config.");
    }
}
...
const char *silvercare_mnn_runtime_kind(void) {
    return "mnn-ios-arm64-official-core";
}
...
char *silvercare_mnn_vision_json_from_chw(
    ...
    try {
        ...
        return copyCString(runYolo(stringOrEmpty(model_dir), chw_rgb, image_width, image_height));
    } catch (...) {
        return nullptr;
    }
}
...
char *silvercare_mnn_text_json(
    ...
    try {
        return copyCString(runLlm(
            stringOrEmpty(model_dir),
            stringOrEmpty(role),
            stringOrEmpty(prompt),
            stringOrEmpty(tuning_config_json),
            static_cast<int>(max_new_tokens),
            stringOrEmpty(end_with)
        ));
    } catch (...) {
        return nullptr;
    }
